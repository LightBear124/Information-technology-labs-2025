...
При работе с Portainer были достигнуты частичные результаты: контейнер Portainer был успешно запущен, однако в процессе настройки через веб-интерфейс по адресу https://127.0.0.1:9000 возникли технические сложности с подключением к Docker socket. Несмотря на предпринятые меры по настройке прав доступа и параметров запуска, полностью завершить настройку Portainer и развернуть стек через веб-редактор не удалось.
